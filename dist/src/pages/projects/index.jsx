...
var ai_1 = require("react-icons/ai"); // Importa o ícone de casa
var Projects = function () {
    ...
    return (<div className="min-h-screen bg-gray-900 text-white relative">
        ...
        <div className="flex items-center">
          <link_1.default href="/" passHref>
            <button className="bg-gray-700 text-white rounded-full p-2 hover:bg-gray-600 transition-colors">
              <ai_1.AiOutlineHome className="w-6 h-6"/>
            </button>
          </link_1.default>
        </div>
      ...
      <main className="mt-20 p-6 overflow-y-auto h-[calc(100vh-6rem)]">
        ...
        {feedbackMessage && (<div className="bg-blue-600 text-white px-4 py-2 rounded-lg mb-4 shadow-lg">
            {feedbackMessage}
          </div>)}

        {/* Lista de Projetos */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects.map(function (project) { return (<div key={project.id} className="bg-gray-800 p-6 rounded-lg flex flex-col items-center justify-center text-center relative shadow-lg">
              <button onClick={function () { return handleDelete(project.id); }} className="absolute top-2 right-2 px-2 py-1 bg-red-600 text-white rounded-lg shadow-lg hover:bg-red-700 transition-colors">
                Excluir
              </button>
            ...
            <form onSubmit={handleCreate} className="bg-gray-800 p-6 rounded-lg shadow-lg w-full max-w-md mb-6">
              <div className="flex justify-between items-center mb-4">
                ...
                <button type="button" onClick={function () { return setFormVisible(false); }} className="text-gray-400 hover:text-gray-600" aria-label="Fechar formulário de criação">
                  <ai_1.AiOutlineClose className="w-6 h-6"/>
                </button>
              </div>

              <div className="mb-4">
                <input type="text" placeholder="Nome do Projeto" value={name} onChange={function (e) { return setName(e.target.value); }} className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-gray-700 text-white"/>
              </div>

              <div className="mb-4">
                <input type="text" placeholder="Descrição do Projeto" value={description} onChange={function (e) { return setDescription(e.target.value); }} className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-gray-700 text-white"/>
              </div>

              <button type="submit" className="w-full px-4 py-2 bg-green-600 text-white rounded-lg shadow-lg hover:bg-green-700 transition-colors">
                Criar Projeto
              </button>
            </form>
          </div>)}
      ...
      {!formVisible && (<button onClick={function () { return setFormVisible(true); }} className={"fixed ".concat(projects.length === 0 ? 'top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2' : 'bottom-10 right-10', " p-4 bg-blue-600 text-white rounded-full shadow-lg hover:bg-blue-700 transition-colors")} aria-label="Mostrar formulário de criação">
